...
class CroMoTEXFinetune(nn.Module):
    def __init__(self, cfg, logger):
        ...
        if len(ckpt_filename) > 0:
            ...
            ) if logger else print(f"Loaded trained checkpoint {ckpt_filename}")
            ...
            logger.info(f"Checkpoint was saved at: {ckpt_time}") if logger else print(f"Checkpoint was saved at: {ckpt_time}")
        else:
...
class CroMoTEXPatchTransformer(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        print(f"Loaded pretrained image encoder from {filepath}")
    ...
    def forward(self, images, ts, finetune=False):
        ts_embeds, *_= self.timeseries_encoder(ts)
        ts_logits = self.classif_head(ts_embeds)
