...
DATASET_BASE_DIR = '/media/omnisky/Disk8.0T/rj/data/MIMIC/processed/' #or '' if local to current project
...
class CXRPretrainDataset(Dataset):
    def __init__(self, cfg, hdf5_file_path, augmentations=None):
        ...
        if isinstance(cfg.pathology, ListConfig):
            self.pathology_index = [pathologies.index(p) for p in cfg.pathology]
        else:
