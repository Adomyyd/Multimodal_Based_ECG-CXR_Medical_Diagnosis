...
def log_epoch_metrics(train_info: dict, val_info: dict, epoch_time) -> str:
    ...
    if 'auroc' in val_info:
        log_str2 = (
            f"val auroc: {', '.join([f'{val_info['auroc'][k]:.4f}' for k in val_info['auroc']])} | "
        )
    else:
        log_str2 = ""
    if 'auprc' in val_info:
        log_str3 = (
            f"val auprc: {', '.join([f'{val_info['auprc'][k]:.4f}' for k in val_info['auprc']])} | "
        )
    else:
        log_str3 = ""
...
def save_log(info, log_name):
    log_dir = hydra.utils.to_absolute_path('logs')
