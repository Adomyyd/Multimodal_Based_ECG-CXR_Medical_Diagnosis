...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str, default='checkpoints/cromotex_finetuned_originECG.pth', help='Path to OriginECG checkpoint')
    parser.add_argument('--batch-size', type=int, default=512, help='Batch size for test dataloader')
...
def build_model(cfg, device, checkpoint_path: str):
    ...
    model = CroMoTEXFinetune(cfg, None)
...
def extract_features(model, dataloader, device):
    ...
    with torch.no_grad():
        for _, ecg, labels in dataloader:
            ecg = ecg.to(device)
            labels = labels.cpu().numpy()

            _, ts_embed= model(ecg)
            feature_ts_list.append(ts_embed.cpu().numpy())
            label_list.append(labels)

    features_ts = np.concatenate(feature_ts_list, axis=0)
    labels = np.concatenate(label_list, axis=0)
    return features_ts, labels
...
def plot_umap(projected, labels, pathology_names, output_dir):
    positive_color = 'red'
    negative_color = 'blue'
    ...
    for idx, pathology in enumerate(pathology_names):
        ...
        plt.title(f'Origin ECG UMAP: {pathology}', fontsize=16)
        plt.xlabel('UMAP 1', fontsize=14)
        plt.ylabel('UMAP 2', fontsize=14)
        plt.tight_layout()
        output_path = os.path.join(output_dir, f'test_ECG_umap_{pathology}.png')
